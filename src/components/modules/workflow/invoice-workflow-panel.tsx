...
import { AnnexureStatus, InvoiceStatus, UserRoleEnum } from "@/utils/constant";
import {
...
export function InvoiceWorkflowPanel({
  ...
  const showSubmit =
    ...
    (status === InvoiceStatus.DRAFT ||
      status === InvoiceStatus.REJECTED_BY_TADMIN ||
      status === InvoiceStatus.REJECTED_BY_BOSS) &&
    annexureStatus === AnnexureStatus.PENDING_TADMIN_REVIEW;
  const showTadminActions =
  ...
  const showAuthorizePayment =
    userRole === UserRoleEnum.BOSS && status === InvoiceStatus.APPROVED;
    ...
    <Card className="border-primary/20 bg-primary/5 ">
      ...
      <CardContent className="  ">
        <div className="flex flex-wrap gap-3">
            ...
            <>
              <Button
                onClick={() => {
                  if (
                    annexureId &&
                    annexureStatus !== AnnexureStatus.APPROVED
                  ) {
          ...
          {userRole === UserRoleEnum.TADMIN &&
            annexureStatus === AnnexureStatus.PARTIALLY_APPROVED && (
        ...
        <AlertDialog
          open={isBossApproveDialogOpen}
          onOpenChange={setIsBossApproveDialogOpen}
        >
          <AlertDialogContent className="max-w-md">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription className="pt-2">
                The linked annexure <strong>{annexureName}</strong> is currently
                in <strong>{annexureStatus}</strong> status. Finance approval
                requires the annexure to be approved first.
              </AlertDialogDescription>
            </AlertDialogHeader>
            ...
            <AlertDialogFooter className="flex-col sm:flex-row gap-2">
              ...
              >
                <Link href={`/lorries/annexure/${annexureId}`}>
                  View Annexure
                </Link>
              </Button>
              <AlertDialogAction
                  ...
                  handleAction(
                    () =>
                      approveInvoiceByBoss(
                        ...
                        userId,
                        userRole,
                        "Directly Approved Both via Invoice",
                      ),
