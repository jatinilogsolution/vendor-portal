...
export function AnnexureWorkflowPanel({
    ...
    const handleBossReject = async (reason: string) => {
        const res = await rejectAnnexureByBoss(annexureId, userId, userRole, reason);
        if (res.success) {
    ...
    const showSubmit = userRole === UserRoleEnum.TVENDOR &&
        (status === AnnexureStatus.DRAFT || status === AnnexureStatus.HAS_REJECTIONS || status === AnnexureStatus.REJECTED_BY_BOSS);

    const showForward = userRole === UserRoleEnum.TADMIN &&
        (status === AnnexureStatus.PARTIALLY_APPROVED || status === AnnexureStatus.PENDING_TADMIN_REVIEW) &&
        allGroupsApproved;

    const showReviewMessage = userRole === UserRoleEnum.TADMIN &&
        (status === AnnexureStatus.PENDING_TADMIN_REVIEW || status === AnnexureStatus.PARTIALLY_APPROVED) &&
        !allGroupsApproved;
        ...
        <Card className="border-primary/20 bg-primary/5">
            ...
            <CardContent className="py-2 ">
                <div className="flex flex-wrap gap-3">
                    {showSubmit && (
                        <Button
                            onClick={status === AnnexureStatus.DRAFT 
                                ? () => handleAction(
                        ...
                        >
                            ...
                            {status === AnnexureStatus.DRAFT ? "Submit for Review" : "Resubmit Annexure"}
                        </Button>
                    )}
                    ...
                    {showReviewMessage && (
                        <Alert className="bg-blue-50 border-blue-200">
                            ...
                            <AlertTitle className="text-blue-800 text-sm">Review in Progress</AlertTitle>
                            <AlertDescription className="text-blue-700 text-xs">
                                Please review and approve each file group before forwarding to BOSS.
                            </AlertDescription>
                        </Alert>
                    )}

                    {status === AnnexureStatus.HAS_REJECTIONS && userRole === UserRoleEnum.TVENDOR && (
                        <Alert variant="destructive" className="bg-red-50 py-2">
                            <AlertCircle className="h-4 w-4" />
                            <AlertDescription className="text-xs">
                                Some file groups have been rejected. Please fix them and resubmit.
                            </AlertDescription>
                        </Alert>
                    )}
                </div>
