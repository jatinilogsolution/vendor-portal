...
export function ManualNotificationDialog({
  ...
  const [isManualEmail, setIsManualEmail] = useState(
    !initialRecipientEmail && availableRecipients.length === 0,
  );

  const isVendor = currentUserRole === UserRoleEnum.TVENDOR;
  const isAdmin = [UserRoleEnum.BOSS, UserRoleEnum.TADMIN].includes(
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Send a custom email notification regarding{" "}
          ...
          {isVendor ? (
            <div className="bg-primary/5 p-4 rounded-xl border border-primary/10 flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <span className="text-sm font-semibold">
                  Recipient: All Admins & Boss
                </span>
                <span className="text-xs text-muted-foreground">
                  This message will be broadcasted to all internal management
                  users.
                </span>
              </div>
            </div>
            ...
            <div className="grid gap-4 bg-muted/30 p-4 rounded-xl border">
              <div className="grid gap-3">
                <Label className="text-[11px] font-bold opacity-70 uppercase tracking-widest px-1">
                  Broadcast To Groups
                </Label>
                <div className="flex flex-wrap gap-4 px-1">
                  <div className="flex items-center gap-2">
                    <Checkbox
                      id="groupBoss"
                      checked={notifyBoss}
                      onCheckedChange={(v) => setNotifyBoss(!!v)}
                  ...
                  <div className="flex items-center gap-2">
                    <Checkbox
                      id="groupTadmin"
                      checked={notifyTAdmin}
                      onCheckedChange={(v) => setNotifyTAdmin(!!v)}
                      className="data-[state=checked]:bg-primary"
                    />
                    <Label
                      htmlFor="groupTadmin"
                  ...
                  <div className="flex items-center gap-2">
                    <Checkbox
                      ...
                      onCheckedChange={(v) => setNotifyVendor(!!v)}
                      className="data-[state=checked]:bg-primary"
                    />
                    <Label
                      htmlFor="groupVendor"
              ...
              <div className="grid gap-2">
                <div className="flex items-center justify-between px-1">
                  <Label className="text-[11px] font-bold opacity-70 uppercase tracking-widest">
                      ...
                      <div className="flex flex-wrap gap-1.5 p-2 bg-background border rounded-lg">
                        {selectedEmails.map((email) => (
                          <Badge
                            key={email}
                            variant="outline"
                            className="flex items-center gap-1 text-[10px] bg-background"
                          >
                            {email}
                            <X
                      ...
                      <SelectContent>
                        {availableRecipients.map((r, i) => (
                          <SelectItem
                            key={i}
                            value={r.email}
                            disabled={selectedEmails.includes(r.email)}
                          >
              ...
              <div className="grid gap-1.5 px-1">
                <Label
                  htmlFor="extra"
                  className="text-[11px] font-bold opacity-70 uppercase tracking-tight flex items-center gap-1.5"
                >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="grid gap-2">
              ...
              <Select value={type} onValueChange={setType}>
                <SelectTrigger
                  id="type"
                  className="bg-muted/50 border-primary/10 h-10"
                >
