...
export async function submitInvoiceForReview(
  ...
  try {
    ...
    if (!invoice.invoiceNumber)
      throw new Error(
        "Missing Invoice Number. Please fill details in 'Manage Invoice' first.",
      );
    if (!invoice.invoiceDate)
      throw new Error(
        "Missing Invoice Date. Please fill details in 'Manage Invoice' first.",
      );
    // if (!invoice.billToId) throw new Error("Missing Bill-To Address. Please fill details in 'Manage Invoice' first.");
    if (!invoice.invoiceURI)
      throw new Error(
        "Missing uploaded Invoice File. Please upload the invoice file first.",
      );
 ...
 */
export async function approveInvoiceByTadmin(
  invoiceId: string,
  userId: string,
  role: string,
  notes?: string,
  ...
  try {
    ...
    if (invoice.annexureId) {
      ...
      if (annexure) {
        const pendingGroups = annexure.groups.filter(
          (g) => g.status !== "APPROVED",
        );
        if (pendingGroups.length > 0) {
          throw new Error(
            `Cannot forward to BOSS: ${pendingGroups.length} annexure file group(s) are not yet approved.`,
          );
        }

        // 🔄 AUTO-FORWARD ANNEXURE: If annexure is still awaiting forwarding, move it along with the invoice
        if (
          annexure.status === AnnexureStatus.PARTIALLY_APPROVED ||
          annexure.status === AnnexureStatus.PENDING_TADMIN_REVIEW
        ) {
            ...
            data: {
              ...
              statusHistory: {
                create: {
                  ...
                  changedBy: userId,
                  notes:
                    "Auto-forwarded to BOSS along with linked Invoice approval",
                },
              },
            },
          });
    ...
    const baseUrl =
      process.env.NEXT_PUBLIC_APP_URL || process.env.NEXT_PUBLIC_API_URL || "";
    const invoiceLink = `${baseUrl}/invoices/${invoiceId}`;
    const autoForwardText = invoice.annexureId
      ? " and linked Annexure auto-forwarded"
      : "";
    const content = [
      `[FORWARDED] Invoice approved by TADMIN${autoForwardText}.`,
      `- **Forwarded To:** BOSS for final review`,
      `- **Invoice:** ${invoice.invoiceNumber || invoice.refernceNumber}`,
      notes ? `- **Remark:** ${notes}` : null,
      `\n[View Invoice Details](${invoiceLink})`,
    ]
      .filter(Boolean)
      .join("\n");
...
export async function rejectInvoice(
  ...
  try {
    ...
    if (
      !canTransitionInvoice(role, invoice.status as InvoiceStatus, targetStatus)
    ) {
        ...
        data: {
          ...
          statusHistory: {
            create: {
              fromStatus:
                (
                  await prisma.annexure.findUnique({
                    where: { id: invoice.annexureId },
                  })
                )?.status || "UNKNOWN",
    ...
    const invoiceLink = `${baseUrl}/invoices/${invoiceId}`;
    const content = [
      `[REJECTED] Invoice has been rejected by ${role === UserRoleEnum.TADMIN ? "TADMIN" : "BOSS"}.`,
      `- **Ref:** ${invoice.invoiceNumber || invoice.refernceNumber}`,
      `> **Reason:** ${reason}`,
      `\n[Review Invoice](${invoiceLink})`,
    ].join("\n");
 ...
 */
export async function approveInvoiceByBoss(
  invoiceId: string,
  userId: string,
  role: string,
  notes?: string,
  ...
  try {
          ...
          data: {
            ...
            statusHistory: {
              create: {
                ...
                changedBy: userId,
                notes: notes
                  ? `Auto-approved via Invoice (${notes})`
                  : "Auto-approved along with linked Invoice final approval (Directly)",
              },
            },
          },
        });
    ...
    const invoiceLink = `${baseUrl}/invoices/${invoiceId}`;
    const content = [
      `[APPROVED] Final approval granted by BOSS.`,
      `- **Invoice:** ${invoice.invoiceNumber || invoice.refernceNumber}`,
      invoice.annexureId
        ? `- **Annexure:** Linked annexure auto-approved`
        : null,
      notes ? `- **Notes:** ${notes}` : null,
      `\n[View Documents](${invoiceLink})`,
    ]
      .filter(Boolean)
      .join("\n");
...
export async function authorizeInvoicePayment(
  ...
  try {
    ...
    const invoiceLink = `${baseUrl}/invoices/${invoiceId}`;
    const content = [
      `[AUTHORIZED] Invoice approved for payment.`,
      `- **Status:** Authorized by BOSS`,
      `- **Ref:** ${invoice.invoiceNumber || invoice.refernceNumber}`,
      `\n[View Details](${invoiceLink})`,
    ].join("\n");
 ...
 */
export async function requestInvoiceDeletion(
  invoiceId: string,
  userId: string,
  role: string,
) {
  try {
    ...
    if (
      ...
      throw new Error(
        "Cannot request deletion: This invoice has already been forwarded to the BOSS/Finance for review.",
      );
    }
