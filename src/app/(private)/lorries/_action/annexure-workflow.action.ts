 ...
 */
export async function submitAnnexureForReview(annexureId: string, userId: string, role: string) {
    try {
        ...
        if (!canTransitionAnnexure(role, annexure.status as AnnexureStatus, AnnexureStatus.PENDING_TADMIN_REVIEW)) {
        ...
        return { success: false, error: error instanceof Error ? error.message : "Failed to submit annexure" };
    }
}
 ...
 */
export async function approveFileGroup(fileGroupId: string, userId: string, role: string) {
    try {
        ...
        if (!canReviewAnnexureGroups(role, group.Annexure.status as AnnexureStatus)) {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            const anyRejected = updatedGroups.some(g => g.status === FileGroupStatus.REJECTED);
            const allApproved = updatedGroups.every(g => g.status === FileGroupStatus.APPROVED);
        ...
        return { success: false, error: error instanceof Error ? error.message : "Failed to approve file group" };
    }
}
 ...
 */
export async function rejectFileGroup(fileGroupId: string, userId: string, role: string, reason: string, affectedLRs?: string) {
    try {
        const group = await prisma.annexureFileGroup.findUnique({
            where: { id: fileGroupId },
            include: {
        ...
        const vendorEmail = firstLR?.tvendor?.contactEmail || firstLR?.tvendor?.users?.[0]?.email;
        ...
        await prisma.workflowComment.create({
            data: {
                content: `REJECTED Group ${group.fileNumber}: ${reason}${affectedLRs ? ` (Affected LRs: ${affectedLRs})` : ""}\nView Annexure: ${annexureLink}`,
                authorId: userId,
        ...
        return { success: false, error: error instanceof Error ? error.message : "Failed to reject file group" };
    }
}
 ...
 */
export async function forwardAnnexureToBoss(annexureId: string, userId: string, role: string) {
    try {
        const annexure = await prisma.annexure.findUnique({
            where: { id: annexureId },
            include: { groups: true }
        ...
        const allApproved = annexure.groups.every(g => g.status === FileGroupStatus.APPROVED);
        if (!allApproved) {
            throw new Error("Cannot forward to BOSS until all file groups are approved");
        }
        ...
        const annexureLink = `${process.env.NEXT_PUBLIC_API_URL}/lorries/annexure/${annexureId}`;
        const linkedInvoice = await prisma.invoice.findFirst({ where: { annexureId: annexureId } });
        await prisma.workflowComment.create({
            data: {
        ...
        return { success: false, error: error instanceof Error ? error.message : "Failed to forward annexure" };
    }
}
 ...
 */
export async function approveAnnexureByBoss(annexureId: string, userId: string, role: string) {
    try {
        const annexure = await prisma.annexure.findUnique({
            where: { id: annexureId }
        });
        ...
        if (!canTransitionAnnexure(role, annexure.status as AnnexureStatus, AnnexureStatus.APPROVED)) {
        ...
        if (linkedInvoice && linkedInvoice.status === InvoiceStatus.PENDING_BOSS_REVIEW) {
        ...
        return { success: false, error: error instanceof Error ? error.message : "Failed to approve annexure" };
    }
}
 ...
 */
export async function rejectAnnexureByBoss(annexureId: string, userId: string, role: string, reason: string) {
    try {
        const annexure = await prisma.annexure.findUnique({
            where: { id: annexureId },
            include: {
        ...
        if (!canTransitionAnnexure(role, annexure.status as AnnexureStatus, AnnexureStatus.REJECTED_BY_BOSS)) {
        ...
        return { success: false, error: error instanceof Error ? error.message : "Failed to reject annexure" };
    }
}
 ...
 */
export async function bulkApproveFileGroups(fileGroupIds: string[], userId: string, role: string) {
    try {
        ...
        await prisma.$transaction(async (tx) => {
            ...
            const anyRejected = allGroups.some(g => g.status === FileGroupStatus.REJECTED);
            const allApproved = allGroups.every(g => g.status === FileGroupStatus.APPROVED);
        ...
        return { success: false, error: error instanceof Error ? error.message : "Failed to bulk approve file groups" };
    }
}
 ...
 */
export async function deleteAnnexure(annexureId: string, userId: string, role: string) {
    try {
        const annexure = await prisma.annexure.findUnique({
            where: { id: annexureId },
        ...
        // Security check
        if (!isVendor) {
            throw new Error("Unauthorized to delete annexure. Only Vendors can delete their own documents.");
        }

        // Restriction check for owner (vendors) - can only delete if not approved/forwarded
        if (isVendor && ![AnnexureStatus.DRAFT, AnnexureStatus.HAS_REJECTIONS, AnnexureStatus.REJECTED_BY_BOSS].includes(annexure.status as AnnexureStatus)) {
            throw new Error("Cannot delete annexure at the current stage. Only DRAFT or REJECTED annexures can be deleted.");
        }

        // Additional restriction: Cannot delete if it is PENDING or APPROVED
        if (![AnnexureStatus.DRAFT, AnnexureStatus.HAS_REJECTIONS, AnnexureStatus.REJECTED_BY_BOSS].includes(annexure.status as AnnexureStatus)) {
             throw new Error("Deletion is blocked. Only DRAFT or REJECTED items can be deleted to maintain data integrity.");
        }
        ...
        return { success: false, error: error instanceof Error ? error.message : "Failed to delete annexure" };
    }
}
